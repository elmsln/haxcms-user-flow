...
export class AppHaxSiteDetails extends SimpleColors {
 ...
 cancelOperation() {
  ...
  async confirmOperation() {
    ...
    const siteId = toJS(store.activeSiteId);
    const site = toJS(store.activeSite);
    // @todo bother to implement these / translate to the path via switch
    //await store.makeCall(op, {site: siteId});
    window.dispatchEvent(new CustomEvent('simple-modal-hide'));
    ...
    return html`
      ...
      <div class="flex-container" id="infomation">
        ...
        <div class="flex-container" id="actions">
            <div class="info-group">
              <simple-icon-button-lite data-site="${this.siteId}" data-site-operation="publish" icon="editor:publish" id="pub" class="info-icon" @click=${this.siteOperation}>
                <div class="info-item">publish</div>
              </simple-icon-button-lite>
              <simple-tooltip for="pub" position="bottom">Publish site</simple-tooltip>
            </div>
            <div class="info-group">
              <simple-icon-button-lite data-site=${this.siteId} data-site-operation="copy" icon="content-copy" id="cp" class="info-icon" @click=${this.siteOperation}>
                <div class="info-item">copy</div>
              </simple-icon-button-lite>  
              <simple-tooltip for="cp" position="bottom">Copy site</simple-tooltip>
            </div>
            <div class="info-group">
              <simple-icon-button-lite data-site=${this.siteId} data-site-operation="download" icon="file-download" id="dl" class="info-icon" @click=${this.siteOperation}>
                <div class="info-item">download</div>
              </simple-icon-button-lite>
              <simple-tooltip for="dl" position="bottom">Download site</simple-tooltip>
            </div>
            <div class="info-group">
              <simple-icon-button-lite data-site=${this.siteId} data-site-operation="delete" icon="delete-forever" id="del" class="info-icon" @click=${this.siteOperation}>
                <div class="info-item">delete</div>
              </simple-icon-button-lite>
              <simple-tooltip for="del" position="bottom">Delete site</simple-tooltip>
            </div>
        </div>
      </div>
    `;
