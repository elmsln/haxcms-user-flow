  ...
  firstUpdated(changedProperties) {
    ...
    autorun(() => {
      if (this.shadowRoot && toJS(store.createSiteSteps) && toJS(store.appReady)) {
        ...
        if (activeItem && activeItem.name && activeItem.step && !this.__overrideProgression) {
          this.shadowRoot
          .querySelector('#link-'.concat(activeItem.name)).click();
        }
      }
    });
  }
            ...
            (item, index) =>
              ...
              class="step ${this.step === item.step ? 'active-step' : ''}">
                ...
                ?disabled="${this.step < item.step || this.step === 5 ? true : false}"
                  ...
                  data-step="${item.step}"
                  >${index+1}</a>
                  <simple-tooltip for="link-${item.name}" position="bottom">Step ${index+1}: ${item.label}</simple-tooltip>
              </li>`
          )}
        </ul>
